...
class Player extends Entity{
    ...
    onTouchMap(tsAhead, tsUnder){
        if(tsAhead===54 || tsUnder===54 || tsUnder===undefined) //каснулись лавы
        {
            this.kill()
            return null; //игрок умер
        }
        if(tsAhead===1 || tsAhead===28) return "land";
...
class Cactus extends Entity{
    ...
    constructor()
    {
        ...
        this.iSshooted = false;
        this.shootToSide=0; //в какую сторону стрелять 0 -нестрелять, 1 - стрелять вправо, -1 - стрелять влево
        this._walkAround=0;
        this.view=0;
        this._lastMove=1;
        this._pos_norm=0;
...
class Bullet extends Entity{
    ...
    onTouchMap(tsAhead,tsUnder){
            if ((tsAhead !== 7 && tsAhead!==undefined) || this.pos_x > getMapManager().mapSize.x || this.pos_x < 0) {
...
class NumBullets{
    ...
    draw()
    {
        getContext().font="italic 15pt Consolas";
        getContext().fillStyle = "blue";
        getContext().fillText("Патроны: "+ this.num.toString(),5,80);
    }
}
...
class Level{
    ...
    draw()
    {
        getContext().font="italic 15pt Consolas";
        getContext().fillStyle = "blue";
        getContext().fillText("Уровень: "+ this.level.toString(),5,40);
    }
}
...
class MyTime
{
    ...
    draw()
    {
        if(!getLoader().firstTime)
        {
            ...
            getLoader().timeResult = "Время: "+min.toString()+":"+sec.toString()+":"+ms.toString();
            getContext().fillText(getLoader().timeResult,5,120);
        }
        else
        {
            getContext().fillText("Время: 00:00:000",5,120);
        }
    }
    update(){}
}
...
class Lifetime{
    ...
    draw(){
        getContext().font="italic 15pt Consolas";
        getContext().fillStyle = "blue";
        getContext().fillText("Жизни: "+ getGameManager().player.lifetime.toString(),5,160);
    }
